import { defaultStyles, notifyDefaults } from "~/static";
...
note.success = (message: string, config?: typeof notifyDefaults) =>
  note.show(message, ENoteAcions.success, config);

note.info = (message: string, config?: typeof notifyDefaults) =>
  note.show(message, ENoteAcions.Info, config);

note.warning = (message: string, config?: typeof notifyDefaults) =>
  note.show(message, ENoteAcions.warning, config);
...
export { note, Notify, Dialog, Loading };
